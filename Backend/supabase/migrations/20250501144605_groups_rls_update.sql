drop policy "Enable update for users based on email" on "groups"."groups";

alter table "groups"."group_members" alter column "joined_at" set not null;
...
for select
to public
using ((( SELECT (get_user_id() = group_pings.from_user_id)) AND ( SELECT (groups.get_user_group() = group_pings.group_id)) AND ( SELECT groups.check_user_group(group_pings.to_user_id, group_pings.group_id) AS check_user_group)));
...
for select
to public
using (( SELECT (groups.get_user_group() = group_subscriptions.group_id)));


create policy "Users can select own group"
on "groups"."groups"
as permissive
for select
to public
