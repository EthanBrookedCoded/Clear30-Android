...
alter table "storage"."iceberg_namespaces" drop constraint "iceberg_namespaces_catalog_id_fkey";

alter table "storage"."iceberg_tables" drop constraint "iceberg_tables_catalog_id_fkey";
...
create policy "Give users access to own folder 1htfcxy_0"
on "storage"."objects"
as permissive
for select
to public
using (((bucket_id = 'testimonial-videos'::text) AND (( SELECT get_user_id() AS get_user_id) = (storage.foldername(name))[1])));


create policy "Give users access to own folder 1htfcxy_1"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'testimonial-videos'::text) AND (( SELECT get_user_id() AS get_user_id) = (storage.foldername(name))[1])));


create policy "Give users access to own folder 1htfcxy_2"
on "storage"."objects"
as permissive
for delete
to public
...
create policy "Give users access to own folder 1htfcxy_3"
on "storage"."objects"
as permissive
for update
to public
