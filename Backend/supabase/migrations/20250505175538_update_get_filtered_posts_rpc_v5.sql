-- This migration updates the get_filtered_posts RPC to v4:
-- 1. Allow post authors to see their own hidden posts
-- 2. Continue to hide posts from all other users
-- 3. Preserves all existing filtering and sorting functionality
...
CREATE OR REPLACE FUNCTION community.get_filtered_posts(
    start_range integer,
    end_range integer,
    tag_ids uuid[] DEFAULT NULL::uuid[],
    only_my_posts boolean DEFAULT false,
    exclude_pinned boolean DEFAULT false,
    min_comments integer DEFAULT 0,
    min_date timestamp DEFAULT NULL,
    sort_by text DEFAULT 'recent'::text  -- Sort options: 'recent', 'views', 'engagement'
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- First, drop any existing grants to avoid conflicts
DO $$
BEGIN
    -- Revoke from authenticated
    BEGIN
        EXECUTE 'REVOKE ALL ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean) FROM authenticated';
    EXCEPTION WHEN OTHERS THEN
        -- Ignore if it doesn't exist
    END;
    
    BEGIN
        EXECUTE 'REVOKE ALL ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean, boolean, integer, text) FROM authenticated';
    EXCEPTION WHEN OTHERS THEN
        -- Ignore if it doesn't exist
    END;
    ...
    -- Revoke from anon
    BEGIN
        EXECUTE 'REVOKE ALL ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean) FROM anon';
    EXCEPTION WHEN OTHERS THEN
        -- Ignore if it doesn't exist
    END;
    
    BEGIN
        EXECUTE 'REVOKE ALL ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean, boolean, integer, text) FROM anon';
    EXCEPTION WHEN OTHERS THEN
        -- Ignore if it doesn't exist
    END;
END $$;
...
GRANT EXECUTE ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean, boolean, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean, boolean, integer, text) TO anon;

-- Add helpful comment
COMMENT ON FUNCTION community.get_filtered_posts(integer, integer, uuid[], boolean, boolean, integer, text) IS 
'Returns filtered posts with pagination and optional filtering by tags, user, pinned status, and comment count.
Supports different sorting methods: recent (default), views, or engagement score.
Hidden posts are visible to their own authors but hidden from other users.';
