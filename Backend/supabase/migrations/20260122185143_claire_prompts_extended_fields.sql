-- Migration: Add claire_prompts_json column for extended claire prompt data
-- This adds a new column that stores the full structured format [{"title", "prompt", "displayPrompt", "emojis"}]
-- Old apps continue using claire_prompts (legacy dict format), new apps use claire_prompts_json

-- Step 1: Add the new column
ALTER TABLE programs.program_messages
ADD COLUMN IF NOT EXISTS claire_prompts_json JSONB;

-- Step 2: Update program_get_messages to return both claire_prompts and claire_prompts_json
        ...
        WITH start_soon_base_data AS (
            SELECT
                ...
                CASE
                    ...
                    ELSE NULL
                END as claire_prompts,
                -- New claire_prompts_json: return directly from column
                pm.claire_prompts_json,
                pm.journal_prompts,
                CASE
                    WHEN pg.id IS NOT NULL THEN
...
-- Step 3: Update fetch_messages_by_ids to return both claire_prompts and claire_prompts_json
drop function if exists "programs"."fetch_messages_by_ids"(message_ids bigint[]);
